...
fn main(@builtin(local_invocation_id) local_id: vec3<u32>) {
    ...
    // (rendering) line. The x coordinate is the local invocation id, which is an index iterating between 0 and 159
    // This means that each workgroup will render a line of 160 pixels.
    let x = local_id.x;
    ...
    let atlasSize = textureDimensions(tileAtlas);
    ...
    let color = textureLoad(tileAtlas, atlas_texel_coord, 0);
